##### 曲面积分
- 曲面积分
	- 曲面积分指在曲面上对一个函数进行积分, 通过对函数值与[[曲面面积]][[微分元素|微分]]的乘积进行求和, 从而得到在整个曲面上的累积量, 当曲面投影到平面上时, 曲面积分变为[[二重积分]]
		- 令 $f$ 为定义在有限区域光滑曲面 $S$ 上的连续标量函数, 那么 $f$ 沿 $S$ 的曲面积分为函数值与曲面面积微分的乘积和
		...
		- 令 $\pmb{F}$ 为定义在有限区域光滑曲面 $S$ 上的[[向量场|三维向量场]], 且曲面具有单位法向量 $\pmb{N}$ 可定向, 则为函数值向量与曲面面积微分向量的[[内积]]和
			- $S:\pmb{r}(u,v) =  f(u,v)\pmb{i}+g(u,v)\pmb{j}+ h(u,v)\pmb{k}$
