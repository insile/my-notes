##### 雅可比矩阵
- 雅可比矩阵
	- 雅可比矩阵是[[函数|多元向量函数]]的一阶[[偏导数]]以一定方式排列成的[[矩阵]], 矩阵本身可以表示基向量的线性坐标变换, 而雅可比矩阵用于近似多元向量函数在某一点附近的非线性变换  
		- 假设某函数 $\pmb{f}: \mathbb{R}^n\to\mathbb{R}^m$ , 从 $x\in\mathbb{R}^n$ 映射到 $\pmb{f}(\pmb{x})\in \mathbb{R}^m$ , 其雅可比矩阵 $\mathbf{J}$ 是一 $m\times n$ 的矩阵, 可以近似表示该函数, 矩阵中行向量即是[[梯度]]转置
			...
			- $\pmb{f}(\pmb{x})\approx\pmb{f}(\pmb{x}_0)+\mathbf{J}(\pmb{x}_0)\cdot(\pmb{x}-\pmb{x}_0)$
